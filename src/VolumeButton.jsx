...
const VolumeButton = ({setcurrentVolume}) => {
  const [volumeValue, setvolumeValue] = useState(50);
  const [volumeModal, setVolumeModal] = useState(true);

  useEffect(() => {
    tickContainer = document.getElementById("tickContainer");
    volumeKnob = document.getElementById("knob");
    boundingRectangle = volumeKnob.getBoundingClientRect(); //get rectangular geometric data of knob (x, y, width, height)
    main();
  }, []);

  useEffect(() => {
    setcurrentVolume(volumeValue/100)
  },[volumeValue])

  function main() {
    volume = 50; //start at zero volume
...
const VolumeModal = ({ volumeValue, volumeModal, setVolumeModal }) => {

  //settimeout to fade away modal here
  return (
    <>
      {volumeModal==true &&
        <div className="modal">
          <div className="volumeValueContainer"><div id="volumeValue" > {volumeValue}</div></div>
        </div>
      }
    </>
  );
